...
BEGIN
            ...
            create TEMPORARY TABLE tmp_product1(id INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY , max_idimp_price INTEGER not NULL, max_idproducthasfile INTEGER not NULL) as (SELECT info3.*, info4.max_idproducthasfile FROM (SELECT info1.*, info2.max_idimp_price FROM (SELECT pc.* FROM (SELECT idproduct from catehasproduct WHERE idcategory = _idcategory) as pc LEFT JOIN products as p on p.idproduct = pc.idproduct WHERE p.idstatus_type = 5 and p.idproduct is not NULL) as info1 LEFT JOIN (SELECT MAX(inf1.idimp) as max_idimp_price, inf1.idproduct from (SELECT idimp, idproduct from imp_products WHERE idstore = _idstore and id_status_type = 4 and idcrosstype IN (0,4)) as inf1 GROUP BY inf1.idproduct) as info2 on info1.idproduct = info2.idproduct WHERE info2.idproduct is not NULL) as info3 LEFT JOIN (SELECT MAX(idproducthasfile) as max_idproducthasfile,idproduct FROM producthasfile WHERE hastype = 1 and status_file = 1 GROUP BY idproduct) as info4 on info3.idproduct = info4.idproduct);
						set _max_order = (SELECT COUNT(tmp1.idproduct) FROM tmp_product1 as tmp1);
						...
						create TEMPORARY TABLE tmp_product3(id INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY , idorder INTEGER not NULL);
                ...
                EXECUTE stmt;
                DEALLOCATE PREPARE stmt;	
								SELECT _max_order as maxcount, tmp2.*, imp1.*, imp2.price as old_price, p.namepro, p.slug, p.short_desc, p.description, phf.idfile, f.urlfile FROM  tmp_product3 as tmp3 LEFT JOIN tmp_product2 as tmp2 on tmp3.idorder = tmp2.id LEFT JOIN imp_products as imp1 on tmp2.max_idimp_price = imp1.idimp LEFT JOIN imp_products as imp2 on imp1.prev_id = imp2.idimp LEFT JOIN products as p on tmp2.idproduct = p.idproduct LEFT JOIN producthasfile as phf on tmp2.max_idproducthasfile = phf.idproducthasfile LEFT JOIN files as f on phf.idfile = f.idfile;
