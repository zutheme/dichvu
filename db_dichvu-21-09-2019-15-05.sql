...
BEGIN
            ...
            create TEMPORARY TABLE tmp_product1(id INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY , idorder INTEGER not NULL);
						...
						set @_end = (_max_order+1)-((_page-1)*9);
						set _start = (_max_order+1) - (_page*9);
						if _start < 1 THEN
							BEGIN
								set _start = 1;
								set @_end = (_max_order+1)%9;
							END;
						END if;
						set @str :='';
						set @comma =',';
