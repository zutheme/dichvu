...
BEGIN
                ...
                DROP TABLE IF EXISTS temp_products;
                set @idorder:=0;
                create TEMPORARY TABLE tmp_product(idtmp INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY , idorder INTEGER not NULL, idcrosstype INTEGER not NULL,parent  INTEGER not NULL,id  INTEGER not NULL, idparentcross INTEGER not NULL,input_quality  INTEGER not NULL,idproduct  INTEGER not NULL,inp_session  INTEGER not NULL,trash  INTEGER not NULL);
                SET @queryString = _str_query;
                PREPARE stmt FROM @queryString;
                EXECUTE stmt;
                DEALLOCATE PREPARE stmt;
          ...
          SELECT tmp.*,imp.* from temp_products as tmp JOIN (select idproduct,price,quality_sale,fromdate,todate from imp_products where idstore = _idstore and id_status_type = 4) as imp on tmp.idproduct = imp.idproduct;
                DROP TABLE tmp_product;
                DROP TABLE temp_products;
            END
;;
...
BEGIN
                ...
                SET @queryString = _queryStr;
                    ...
                    INSERT into exp_products(idnumberorder, idcrosstype, idproduct,idorder, parentidorder, idcustomer, idrecipent, iduser, amount, price, note, idstore, axis_x, axis_y, axis_z, id_status_type) select _idnumberorder, info.idcrosstype, info.idproduct, info.idorder, info.parent, _idcustomer, _idrecipent, _iduser, info.inp_session, info.price,'' as note, _toidstore, _axis_x, _axis_y, _axis_z, info.trash from (SELECT info_p.*,imp.price, imp.quality_sale FROM temp_products as info_p JOIN (select idproduct,price, quality_sale from imp_products WHERE idstore = _fromidstore and id_status_type = 4) as imp on info_p.idproduct = imp.idproduct) as info;
