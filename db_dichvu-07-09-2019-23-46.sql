...
) ENGINE = InnoDB AUTO_INCREMENT = 165 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci ROW_FORMAT = Compact;
...
delimiter ;;
CREATE PROCEDURE `OrderProductFromSessionProcedure`(IN `_idcustomer` INT(11), IN `_idrecipent` INT(11), IN `_iduser` INT(11), IN `_note` TEXT CHARSET utf8mb4, IN `_namestore` VARCHAR(255),  IN `_queryString` VARCHAR(255))
BEGIN
                Declare _idcattype int;
                DECLARE _idstore int;
                ...
                SET @queryString = _queryString;
                    ...
                    INSERT into exp_products(idnumberorder, idcrosstype, idproduct,idorder, parentidorder, idcustomer, idrecipent, iduser, amount, price, note, idstore, axis_x, axis_y, axis_z, id_status_type) select _idnumberorder, info.idcrosstype, info.idproduct, info.idorder, info.parent, _idcustomer, _idrecipent, _iduser, info.inp_session, (CASE WHEN info.idcrosstype = 1 THEN info.price_combo WHEN info.idcrosstype = 2 THEN info.price_gift ELSE info.price END) as price_checkout,'' as note, _idstore, _axis_x, _axis_y, _axis_z, info.trash from (SELECT info_p.*,imp.price, imp.price_sale_origin, imp.price_combo, imp.quality_combo, imp.price_gift,imp.quality_gift FROM temp_products as info_p JOIN (select idproduct,price, price_sale_origin, price_combo, quality_combo, price_gift,quality_gift from imp_products WHERE idstore = _idstore) as imp on info_p.idproduct = imp.idproduct) as info;
                    DROP TABLE tmp_product;
                    DROP TABLE temp_products;
            END
;;
