 ...
 DROP TABLE IF EXISTS tmp_product1;
                ...
                create TEMPORARY TABLE tmp_product2(idtmp INTEGER NOT NULL AUTO_INCREMENT PRIMARY KEY , idorder INTEGER not NULL, idcrosstype INTEGER not NULL, parent  INTEGER not NULL, idparentcross INTEGER NULL,input_quality  INTEGER not NULL,idproduct  INTEGER not NULL,inp_session  INTEGER not NULL,trash  INTEGER not NULL) as (SELECT * from tmp_product1);
								(SELECT tmp1.*,inf1.* FROM tmp_product1 as tmp1 LEFT JOIN (select idimp, idproduct, idcrosstype, idparentcross, amount, price_import, price, quality_sale from imp_products WHERE idcrosstype = 0 and idstore = @_idstore and id_status_type = 4 and idparentcross = 0) as inf1 on tmp1.idproduct = inf1.idproduct and tmp1.idparentcross = inf1.idparentcross WHERE inf1.idproduct is not NULL UNION all SELECT tmp2.*,inf2.* FROM tmp_product2 as tmp2 LEFT JOIN (select idimp, idproduct, idcrosstype, idparentcross, amount, price_import, price, quality_sale from imp_products WHERE idcrosstype = 4 and idstore = @_idstore and id_status_type = 4 and idparentcross > 0) as inf2 ON tmp2.idproduct = inf2.idproduct;
